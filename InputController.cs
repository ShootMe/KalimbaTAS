...
namespace KalimbaTAS {
    public class InputController {
        ...
        private int currentFrame, inputIndex, frameToNext, fastForward;
        ...
        public bool FastForwarding { get { return fastForward > currentFrame; } }
        public bool EndFastForwarding { get { return fastForward == currentFrame; } }
        public InputRecord CurrentInput { get { return lastInput; } }
            ...
            using (StreamReader sr = new StreamReader(filePath)) {
                while (!sr.EndOfStream) {
                    ...
                    if (line.IndexOf("Read", System.StringComparison.OrdinalIgnoreCase) == 0 && line.Length > 5) {
                        lines++;
                        ReadFile(line.Substring(5), lines, totalFrames);
                        lines--;
                    }

                    if (line == "***") {
                        fastForward = totalFrames;
                    }
        ...
        private int ReadFile(string extraFile, int lines, int totalFrames) {
            ...
            if (!File.Exists(filePath)) { return totalFrames; }
            ...
            using (StreamReader sr = new StreamReader(filePath)) {
                while (!sr.EndOfStream) {
                    ...
                    if (line.IndexOf("Read", System.StringComparison.OrdinalIgnoreCase) == 0 && line.Length > 5) {
                        totalFrames = ReadFile(line.Substring(5), lines, totalFrames);
                    }
